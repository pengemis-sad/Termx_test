// jgn di apus nih wm avosky ngotak ya 
...
async function startSpam() {
    ...
    const { state: authState, saveCreds: saveCredentials } = await useMultiFileAuthState('./Sanzz');
    ...
    if (pairingCode && !waSocket.authState.creds.registered) {
        ...
        let phoneNumber = await askQuestion(chalk.bgBlack(chalk.whiteBright('Input NO Whatsapp: +628xxx : ')));
        phoneNumber = phoneNumber.replace(/[^0-9]/g, '');

        while (!Object.keys(PHONENUMBER_MCC).some(prefix => phoneNumber.startsWith(prefix))) {
            console.log(chalk.bgBlack(chalk.yellowBright('')));
            console.log(chalk.bgBlack(chalk.blueBright('')));
            console.log(chalk.bgBlack(chalk.redBright('Masukkan nomor telepon yang valid.')));
            phoneNumber = await askQuestion(chalk.bgBlack(chalk.greenBright('Input NO Whatsapp: +628xxx : ')));
        }
